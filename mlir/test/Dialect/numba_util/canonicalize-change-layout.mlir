...
func.func @test_change_layout(%arg : memref<?xi32>) -> memref<?xi32, #map> {
  %0 = numba_util.change_layout %arg : memref<?xi32> to memref<?xi32, #map>
...
func.func @test_change_layout(%arg : memref<?xi32, #map>) -> index {
  %0 = numba_util.change_layout %arg : memref<?xi32, #map> to memref<?xi32>
...
//       CHECK:   %[[RES:.*]]  = numba_util.change_layout %[[CLONE]] : memref<?xi32, #[[MAP]]> to memref<?xi32>
//       CHECK:   return %[[RES]]
#map = affine_map<(d0)[s0, s1] -> (s0 + s1 * d0)>
...
//       CHECK:   %[[RES:.*]]  = numba_util.change_layout %[[SELECT]] : memref<?xi32, #[[MAP]]> to memref<?xi32>
//       CHECK:   return %[[RES]]
#map = affine_map<(d0)[s0, s1] -> (s0 + s1 * d0)>
...
func.func @test_change_layout(%arg1 : memref<?xi32>, %arg2 : memref<?xi32, #map>, %arg3 : i1) -> memref<?xi32> {
  %0 = numba_util.change_layout %arg2 : memref<?xi32, #map> to memref<?xi32>
...
//       CHECK:   %[[RES2:.*]] = numba_util.change_layout %[[RES1]] : memref<?x?xf64, strided<[1, 1], offset: ?>> to memref<?x?xf64>
...
func.func @test_change_layout(%arg1 : memref<?x?xf64, strided<[1, 1], offset: ?>>, %arg2 : memref<?x?xf64, strided<[1, 1], offset: ?>>, %arg3 : i1) -> memref<?x?xf64> {
  %0 = scf.if %arg3 -> (memref<?x?xf64>) {
    %1 = numba_util.change_layout %arg1 : memref<?x?xf64, strided<[1, 1], offset: ?>> to memref<?x?xf64>
    scf.yield %1 : memref<?x?xf64>
  } else {
    %2 = numba_util.change_layout %arg2 : memref<?x?xf64, strided<[1, 1], offset: ?>> to memref<?x?xf64>
...
// CHECK-LABEL: func @test_change_layout
//       CHECK:   %[[RES1:.*]] = numba_util.env_region "test" -> memref<?xi32, strided<[?], offset: ?>> {
//       CHECK:   %[[VAL:.*]] = "test.test"() : () -> memref<?xi32, strided<[?], offset: ?>>
//       CHECK:   numba_util.env_region_yield %[[VAL]] : memref<?xi32, strided<[?], offset: ?>>
//       CHECK:   }
//       CHECK:   %[[RES2:.*]] = numba_util.change_layout %[[RES1]] : memref<?xi32, strided<[?], offset: ?>> to memref<?xi32>
//       CHECK:   return %[[RES2]] : memref<?xi32>
func.func @test_change_layout() -> memref<?xi32> {
  %0 = numba_util.env_region "test" -> memref<?xi32> {
    %1 = "test.test"() : () -> memref<?xi32, strided<[?], offset: ?>>
    %2 = numba_util.change_layout %1 : memref<?xi32, strided<[?], offset: ?>> to memref<?xi32>
    numba_util.env_region_yield %2: memref<?xi32>
  }
