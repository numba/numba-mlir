...
struct OCLLoader {
  using clCreateProgramWithIL_T = cl_program (*)(cl_context, const void *,
                                                 size_t, cl_int *);
  using clReleaseProgram_T = cl_int (*)(cl_program);
  using clBuildProgram_T = cl_int (*)(cl_program, cl_uint, const cl_device_id *,
                                      const char *,
                                      void (*)(cl_program, void *), void *);
  using clGetProgramBuildInfo_T = cl_int (*)(cl_program, cl_device_id,
  ...
  OCLLoader() : dynLib(clLibName) {
    clCreateProgramWithIL =
        dynLib.getSymbol<clCreateProgramWithIL_T>("clCreateProgramWithIL");
    ...
    clGetProgramBuildInfo =
        dynLib.getSymbol<clGetProgramBuildInfo_T>("clGetProgramBuildInfo");
  ...
  if (backend == ze_be) {
    ...
    auto kernelBundle =
        sycl::make_kernel_bundle<ze_be, sycl::bundle_state::executable>(
            {l0module.get()}, ctx);
    return new GPUModule{&queue, std::move(kernelBundle), std::move(l0module),
                         nullptr};
  }
  if (backend == cl_be) {
